...
basin_ids = ['VIDO3']#['BOOT1','BRVG1','CHAN7','CHEN7','CRKT1','FPHT1','HADT1','HIWN7','HWEG1','JSST1','MPHN7','NOTG1','SNTN7']
#sim_type = 'final' # choices: initial (prior to calib) or final (final calib)
RFC = 'NWRFC'
error_types = ['bias','pbias'] # choices: pbias, bias, NAE (normalized absolute error)
...
for basin_id in basin_ids:
    print basin_id
    calib_read = open('P:\\NWS\\'+RFC+'\\Calibration_TimeSeries\\' + basin_id + '_Simulation_TimeSeries.csv', 'r') #test!!!
    # output figure directory
    out_dir = 'P:\\NWS\\'+RFC+'\\Calibration_TimeSeries\\'
    if basin_id == 'BLUO3':
        yr_start = 1970; yr_end = 2003 #
        cminb = -100; cmaxb = 100
    if basin_id == 'CGRO3':
        yr_start = 1970; yr_end = 2003 #
        cminb = -100; cmaxb = 100
    ...
    test = pd.read_csv(calib_read,sep=',',skiprows=2,
            usecols=[0,call_QME,call_obs],parse_dates=['date'],names=['date', 'OBS', 'QME'])
    ...
    if add_obs_Q_plot == 'yes':
        ...
        fig_out = out_dir +  basin_id + '_bias_pbias' + '_subplots.png'
    else:
        fig_out = out_dir + basin_id + '_bias_pbias' + '.png'
    plt.savefig(fig_out, dpi=350, bbox_inches='tight')
