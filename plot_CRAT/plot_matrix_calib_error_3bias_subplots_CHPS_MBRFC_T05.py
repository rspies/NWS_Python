...
basin_ids = ['BULW4'] # <-- use this to run specific basins
########### loop through all desired basins and define min/max errors for plots ############
for basin_id in basin_ids:
    print basin_id
    calib_read = open(maindir + RFC[:5] + os.sep + RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type+ os.sep + 'QME_SQME' + os.sep + basin_id + '_Simulation_TimeSeries.csv', 'r') #test!!!
    # find the number of columns in the csv to call SQME/QME variables below (last two columns)   
    for line in calib_read:
        cols = len(line.split(','))
        calib_read.close()
        break    
    calib_read = open(maindir + RFC[:5] + os.sep + RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type+ os.sep + 'QME_SQME' + os.sep + basin_id + '_Simulation_TimeSeries.csv', 'r') 
    ...
    test = pd.read_csv(calib_read,sep=',',skiprows=2,
            usecols=[0,cols-2,cols-1],parse_dates=['date'],names=['date', 'SQME', 'QME'])
