...
yr_start  = 2000; yr_end = 2013 # specify the first and last year to analyze
csv_loc = maindir + os.sep + RFC[:5] + os.sep + RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type + os.sep + 'QIN_SQIN' + os.sep
...
for basin_id in basin_ids:
    ...
    calib_read = open(maindir + os.sep + RFC[:5] + os.sep + RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type + os.sep +'QIN_SQIN' + os.sep + basin_id + '_Hourly_Simulation_TimeSeries.csv', 'r') #test!!!
    ...
    if basin_id == 'CTKT2':
        step = 1    # time step (1 or 6 hour)
        period = 18 # number of time steps to examine before and after event peak
        thresh= 5 # cutoff for events to examine
    elif basin_id == 'CMKT2' or basin_id == 'TNGT2':
        step = 1    
        period = 18 
        thresh= 100 
    else:
    ...
    test = pd.read_csv(calib_read,sep=',',skiprows=2,
            usecols=[0,cols-3,cols-1],parse_dates=['date'],names=['date', 'SQIN', 'OBS'])
        ...
        if obs > thresh and actual_index >= period:
            ...
            if obs >= max(date_Q[actual_index + 1:actual_index + period]) and obs > max(date_Q[actual_index - period: actual_index]):
                ...
                if status == 'ok':
                    ...
                    tot_events += 1
                    print str(new_date[actual_index])
