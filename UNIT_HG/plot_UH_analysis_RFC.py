...
for basin_id in basin_ids:
    ...
    std_flow = np.std(date_Q_calib)
    print 'Mean flow: ' + str(avg_flow)
    if avg_flow != avg_flow: # check if data is available 
        print 'No observed data available... skipped basin'
    else: # check if data is available 
        thresh = int(round(avg_flow + (2*std_flow),0))
    #    if 0 <= avg_flow <= 10:
    #        thresh = 40
    #    elif 10 <= avg_flow <= 20:
    #        thresh = 60
    ...
    #        thresh = 400   
        ...
        for obs in date_Q:
            ...
            if obs > thresh and actual_index >= period and actual_index < (len(date_Q) - period):
