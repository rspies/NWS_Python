...
RFC = 'MBRFC_FY2015'
resolution = 450 #350->(for report figs) 100->for CHPS fx help tab display (E19)
...
csv_loc = maindir + os.sep + RFC[:5] + os.sep + RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type + os.sep + 'QME_SQME' + os.sep
out_dir = maindir + os.sep + RFC[:5] + os.sep + RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type+ os.sep + 'UH_analysis_plots' + os.sep
out_list = maindir + os.sep + RFC[:5] + os.sep + RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type+ os.sep + 'UH_analysis_plots' + os.sep + 'uhg_event_analysis.txt'
step = 24 # data time step
period = 6
################  Define the corresponding column of data in the csv file  #################
# create a text file with a list of events analyzed for each basin
write_date_list = open(out_list,'w') 

ignore_basins = ['CASM8','ULLM8','CLEM8']
if basin_ids == []:
...
############################################################            
high_flows = ['FBNM8','GFLM8','ULMM8','VRGM8','LDKM8']
midhigh_flows = ['HLTM8','ULMM8']
mid_flows = ['ADDM8','BULW4','DUBW4','EDNM8','SMHM8']
midlow_flows = ['DINW4']
low_flows = ['DCKW4','MDYM8','BTCM8','TSFM8','SSNM8','LPPM8','WLLM8']
###########################################################
for basin_id in basin_ids:
    print basin_id
    write_date_list.write(basin_id + '\t')
    calib_read = open(csv_loc + basin_id + '_Simulation_TimeSeries.csv', 'r') #test!!!
    # find the number of columns in the csv to call variables below    
    for line in calib_read:
        cols = len(line.split(','))
        calib_read.close()
        break    
    calib_read = open(csv_loc + basin_id + '_Simulation_TimeSeries.csv', 'r') 
    ...
    if basin_id in high_flows:
        ...
        thresh= 1000 # cutoff for events to examine
    ...
    test = pd.read_csv(calib_read,sep=',',skiprows=2,
            usecols=[0,cols-2,cols-1],parse_dates=['date'],names=['date', 'SQIN', 'OBS'])
    ...
    if step == 24:
        ax1.bar(x_labels, array_pbias, color='b', width=10,align='center')
    else:
        ax1.bar(x_labels, array_pbias, color='b', align='center')
    ...
    ax2.set_ylabel('Mean Event Series (CMSD)',fontsize=7)
