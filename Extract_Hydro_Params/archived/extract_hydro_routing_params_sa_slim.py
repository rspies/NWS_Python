...
snow_plots = 'on' # choices: 'on' or 'off' -> areal depleation plots
...
if param_source == 'pre_calb':
    folderPath = maindir + '\\CHPS\\chps_calb\\FY'+RFC[-4:]+'\\' + RFC[:5].lower() + '_calb\\Config\\ModuleParFiles\\*'
if param_source == 'sa':
    folderPath = maindir + '\\CHPS\\chps_sa\\FY'+RFC[-2:]+'\\' + RFC[:5].lower() + '_sa\\Config\\ModuleParFiles\\*'
...
if sacsma == 'on':
    ...
    for filename in glob.glob(os.path.join(folderPath, "SAC*.xml")):
        ...
        xml_file_open = open(filename, 'r')
        #loop through parameters in the 'sac_params' list
        for i,param in enumerate(sac_params):
            
            #get line number that the paramter is on
            line_num = -1
            xml_file = xml_file_open.readlines()
            for line in xml_file:
                line_num += 1
                if param in line and 'dbl' not in line and 'bool' not in line:
                    next_line = xml_file[line_num + 1]
                    print 'found param'
                    print line
                    print line_num
                    print next_line
                    if 'dblValue' in next_line or 'intValue' in next_line or 'boolValue' in next_line:
                        case = 1
                    elif 'description' in next_line:
                        case = 3
                    else:
                        print 'Unrecognized moduleparamfile formatt...' + param
                    break
                elif param in line and 'dbl' in line and 'bool' not in line:
                    case = 2
                    break
            ...
            if param not in xml_file:
                ...
                #xml_file.seek(0)
                if case == 1:
                    section = xml_file[line_num:line_num+1] #normal next line format
                    print line_num
                    print section
                elif case == 2:
                    section = xml_file[line_num-1:line_num] #same line format
                elif case == 3:
                    section = xml_file[line_num+1:line_num+2] #exported mod format
                ...
                for line in section:
                    ...
                    csv_file.write(line + ',')
                    if line == '':
                        print 'Warning - null value for parameter: ' + param
        ...
        while x <12:    
            ...
            section = xml_file[line_num_start:line_num_end]
...
if snow == 'on':
    ...
    for filename in glob.glob(os.path.join(folderPath, "SNOW*.xml")):
        ...
        for i,param in enumerate(snow_params):
    
            #Find line number with parameter string
            #Line number is saved when loop breaks
            line_num = 0
            for line in xml_file:
                line_num += 1
                if param in line and 'dbl' not in line and 'bool' not in line:
                    if 'dblValue' in xml_file.next() or 'intValue' in xml_file.next or 'boolValue' in xml_file.next:
                        case = 1
                    elif 'description' in xml_file.next():
                        case = 3
                    else:
                        print 'Unrecognized moduleparamfile formatt...'
                    break
                elif param in line and 'dbl' in line and 'bool' not in line:
                    case = 2
                    break
            ...
            if param not in xml_file.read():
                ...
                xml_file.seek(0)
                if case == 1:
                    section = xml_file.readlines()[line_num:line_num+1] #normal next line format
                elif case == 2:
                    section = xml_file.readlines()[line_num-1:line_num] #same line format
                elif case == 3:
                    section = xml_file.readlines()[line_num+1:line_num+2] #exported mod format
