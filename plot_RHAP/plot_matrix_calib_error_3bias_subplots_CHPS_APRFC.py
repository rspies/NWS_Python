...
resolution = 100 #350->(for report figs) 100->for CHPS fx help tab display (E19)
################  Define the corresponding column of data in the csv file  #################

yr_start = 1975; yr_end = 2013 #

########### find all basin QME vs SQME .csv files ############
if len(basin_ids) == 0:
    basin_ids = []
    all_files = os.listdir(maindir + RFC+ os.sep + 'Calibration_TimeSeries'+ os.sep + sim_type + os.sep + 'QME_timeseries' + os.sep)
...
for basin_id in basin_ids:
    print basin_id
    calib_read = open(maindir +RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type+ os.sep + 'QME_timeseries' + os.sep + basin_id + '_Simulation_TimeSeries.csv', 'r') #test!!!
    # output figure directory
    out_dir = maindir +RFC + os.sep + 'Calibration_TimeSeries' + os.sep + sim_type + os.sep + 'raster_hydrograph_plots' + os.sep + 'E19' + os.sep
    if basin_id == 'CJXA2':
        cminb = -2000; cmaxb = 2000
        cmina = -50000; cmaxa =50000
        call_SQME = 7; call_QME = 8
        yr_start = 1975; yr_end = 2013
    elif basin_id == 'MCGA2':
        cminb = -1000; cmaxb = 1000
        cmina = -30000; cmaxa =30000
        call_SQME = 7; call_QME = 8
        yr_start = 1990; yr_end = 2013
    elif basin_id == 'KLCA2':
        cminb = -1000; cmaxb = 1000
    ...
    test = pd.read_csv(calib_read,sep=',',skiprows=2,
            usecols=[0,call_SQME,call_QME],parse_dates=['date'],names=['date', 'SQME', 'QME'])
