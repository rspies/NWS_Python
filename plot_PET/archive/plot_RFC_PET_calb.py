...
############################### User Input ####################################
rfc = 'MBRFC'
plot_type = 'calb' # calb looks for initial and calb files
csv_read_NWS = open(maindir + os.sep + 'MBRFC_Plot_PET_Initial_NWS.csv', 'r')
csv_read_FAO = open(maindir + os.sep + 'MBRFC_Plot_PET_Initial_FAOPM.csv', 'r')
...
############################ End User Input ###################################
if plot_type == 'calb':
    csv_read_calb = open(maindir + os.sep + 'MBRFC_Plot_PET_final_calb.csv', 'r')
    data_calb = pd.read_csv(csv_read_calb, delimiter=',', skip_footer=0, header=0)
...
fig.suptitle('Final Mid-month ET-Demand Comparison'+' (Units = mm/day)',y=1.02,fontsize=15)
...
for basin in all_basins:
    if basin != 'month' and basin != 'Month':
        ...
        ax1.plot(x,y1, color='black', markersize=4, linestyle='-', linewidth=1, zorder=5, label = rfc + ' Calb')
...
ax1.legend(loc="center",shadow=True,fontsize=15,bbox_to_anchor=(-0.1, 1.3))
