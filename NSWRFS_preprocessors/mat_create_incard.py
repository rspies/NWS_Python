...
networks = ['asos_hourly','nhds_daily','raws_hourly']  # choices: 'asos_hourly','nhds_daily','raws_hourly','scan_hourly'
...
## define basins below
mat_basins = ['MCGA2GL','MCGA2UPR','MCGA2LWR','KLCA2UPR','KLCA2LWR','CJXA2GL','CJXA2UPR','CJXA2LWR','ANKA2UPR','ANKA2LWR']
mat_basins_desc = {'MCGA2UPR':'McGrath_Upper','MCGA2LWR':'McGrath_Lower','MCGA2GL':'McGrath_Glacier','KLCA2UPR':'Liskys_Upper','KLCA2LWR':'Liskys_Lower',
              'CJXA2LWR':'Crooked_Lower','CJXA2UPR':'Crooked_Upper','CJXA2GL':'Crooked_Glacier','ANKA2LWR':'Aniak_Lower','ANKA2UPR':'Aniak_Upper'}
mat_basins_area = {'MCGA2UPR':4578,'MCGA2LWR':6753,'MCGA2GL':234,'KLCA2UPR':1028,'KLCA2LWR':3050,
              'CJXA2LWR':8846,'CJXA2UPR':6465,'CJXA2GL':143,'ANKA2LWR':2503,'ANKA2UPR':1905}
dummy_file = workingdir + os.sep + 'MAT_input' + os.sep + 'temp_weights' + os.sep + 'dummy_input_info_04242015.csv'
########################################################
################## end user input #######################

if len(networks) > 1:
    out_file = open(workingdir + os.sep + 'MAT_input' + os.sep + 'MAT_KUSKO_BASINS_final.mat','wb')
else:
    out_file = open(workingdir + os.sep + 'MAT_input' + os.sep + 'MAT_' + networks[0] + '.mat','wb')
print 'Creating file -> ' +  str(out_file)
################ A card block ####################
month1 = 10; year1 = 1960; month2 = 9; year2 = 2013
out_file.write('{:2s} {:2d} {:4d} {:2d} {:4d}'.format('@A',month1,year1,month2,year2))
out_file.write('\n')
################ B card block ####################
MAT_compute = len(mat_basins); tempck_num = 0; MAT_units = 'ENGL'; mnt = 'MNT'
ctmp = 'CTMP'; ctim = 'CTIM'; cons = ',,'; cont = 'CONT' # note: need n+1 commas for defaults (n is number of default variables)
...
# ignored for TEMPCK runs and consistency checks without MAT
#if tempck_num > 0 or MAT_compute > 0: 
print_option = ','; null = ',,'; output_MAT = 'OUT'; MAT_units = 'ENGL'; dir_name = 'KUSKO_BASINS'; summary_table = 'SUMT'; summary_card = 'SUMP'
...
for network in networks:
    if network == 'nhds_daily': # nhds_daily.taplot vs asos.taplot naming difference
        count_open = open(workingdir + os.sep + 'taplot_input' + os.sep + network + '.taplot','r')
    else:
        count_open = open(workingdir + os.sep + 'taplot_input' + os.sep + network[:4] + '.taplot','r')
...
for network in networks:
    line_prev = ''
    if network == 'nhds_daily': # nhds_daily.taplot vs asos.taplot naming difference
        taplot_open = open(workingdir + os.sep + 'taplot_input' + os.sep + network + '.taplot','r')
        summary_open = open(workingdir + os.sep + 'nhds_summary_tamx_daily.csv','r')
    else:
        taplot_open = open(workingdir + os.sep + 'taplot_input' + os.sep + network[:4] + '.taplot','r')
        summary_open = open(workingdir + os.sep + network[:4] + '_summary_tamx_hourly.csv','r')
...
for mat_basin in check_basins:
    basin_index += 1
    area_id = mat_basin; area_desc = mat_basins_desc[mat_basin]; area = mat_basins_area[mat_basin]; area_units = 'MI2'; basin_name = 'KUSKO_BASINS'; file_name = area_id
...
for network in networks:
    if network == 'nhds_daily':
        for each_file in (os.listdir(workingdir + os.sep + network + os.sep + 'tamx' + os.sep + 'cardfiles')):
            station_list[each_file[3:7]] = (each_file[:-4])

    else:
        for each_file in (os.listdir(workingdir + os.sep + network + os.sep + 'cardfiles_temp')):
