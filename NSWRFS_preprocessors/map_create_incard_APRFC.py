...
for map_basin in map_basins:
    out_file = open(workingdir + os.sep + 'MAP_input' + os.sep + '1960-2015' + os.sep + 'MAP_input_'+map_basin+'_1960_2015.map','wb')
    pxpp_output = workingdir + os.sep + 'MAP_input' + os.sep + '1960-2015' + os.sep + 'pxpp_punch' + os.sep + 'ptpx_' + fxgroup + '_breaks_pun.txt' # file with the pxpp output -> template for MAP input
    print 'Creating file -> ' +  str('MAP_input_'+map_basin+'_1960_2015.map')
################ A card block ####################
# data pulled from pxpp output punch file
    ...
    open_pxpp.close()
################ B card block ####################
    MAP_compute = 0; station_weight = 'PREE'; null = '0'; cont = 'CONT'; interval = '6'; adj = 'ADJ'; cons = 'SESN'
...
# area information and predetermined weights
    if MAP_compute > 0: # only add @I card for MAP generation run - not consistency check run
        area_id = map_basin; area_desc = map_basins_desc[map_basin].replace(' ','_'); area = map_basins_area[map_basin]; area_units = 'MI2'; basin_name = 'FY17 CALB BASIN'; file_name = area_id
