...
basins_folder = 'P:\\NWS\\GIS\\' + RFC + '\\shapefiles\\catchments'

# if you only want to run specific basins -> list them below
# otherwise set it equal to empty list (basins_overwrite = [])
basins_overwrite = [] 

output_dir = 'P:\\NWS\\GIS\\' + RFC + '\\PRISM\\Catchments\\Model_Builder_Output_' + variable +'_month\\'  # this must contain a folder for each basin (eg. FONN7)
...
basin_files = glob.glob(basins_folder+'/*.shp') # list all basin shapefiles in the above specified directory
...
for each in basin_files:
    if each[-10:-4] not in basins:
        basins.append(each[-10:-4])

print 'Identified ' + str(len(basins)) + ' basins in ' + RFC + ' input directory...'        

for month in months:
    # location of PRISM Raster (CONUS)
    PRISM_Dataset = 'Q:\\GISLibrary\\PRISM\\Alaska\\AK_PRISM_' + variable + '\\' + variable + '\\' + variable + months[month]
