...
################### User Input #####################
RFC = 'NCRFC_FY2016'
fx_group = 'DES' # leave blank if not processing by fx group
variable = 'tmin' # use temperature: 'tmean','tmax','tmin' or precipitation: 'ppt'
#in_shp = maindir + '\\' + RFC[:5] + os.sep + RFC + '\\Shapefiles_from' + RFC[:5] + '\\calb_basins\\calb_basins_DES.shp'
in_shp = maindir + '\\' + RFC[:5] + os.sep + RFC + '\\Shapefiles_fromNCRFC\\calb_basins\\' + 'Bighorn_Yellowstone_calb_basins.shp'
...
if fx_group != '':
    output_dir = maindir + '\\'+ RFC[:5] + os.sep + RFC + '\\PRISM\\Model_Builder_Output_' + variable +'_month\\' + fx_group +os.sep
else:
    output_dir = maindir + '\\'+ RFC[:5] + os.sep + RFC + '\\PRISM\\Model_Builder_Output_' + variable +'_month\\'
